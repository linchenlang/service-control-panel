...
# ---------- IP 封禁模块导入 ----------
try:
    from ip_ban import banned_ips, banned_records, block_ip, unblock_ip, load_banned_ips
    ban_records = banned_records
except ImportError:
    banned_ips = set()
    banned_records = {}
    ban_records = {}
    def block_ip(ip, service_id): pass
    def unblock_ip(ip, service_id): pass
    def load_banned_ips(): pass

# ---------- 风险检测模块导入 ----------
try:
    from risk_detector import get_risks_cached
except ImportError:
...
def api_deployment_config():
    ...
    if request.method == "GET":
        ...
        config["deployment"] = {
            ...
            "auto_ban_ip": data.get("auto_ban_ip", True),
