...
async function showIpconfigDetail() {
    ...
    try {
        const res = await fetch('/api/ipconfig_raw');
        const data = await res.json();
        if (data.success) {
...
function attachServiceEvents() {
    ...
    document.querySelectorAll('.btn-maintenance').forEach(btn => btn.onclick = async () => {
        ...
        const res = await fetch(`/api/services/${id}/maintenance`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({enabled: !currently}) });
        const data = await res.json();
...
async function saveSettings() {
    ...
    const res = await fetch('/api/settings', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({panel_host: host, panel_port: port}) });
    const data = await res.json();
