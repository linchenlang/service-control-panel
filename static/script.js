...
// ---------- 弹窗队列 ----------
let alertModalQueue = [];
let isAlertModalShowing = false;

function showAlertModal(message) {
    alertModalQueue.push(message);
    if (!isAlertModalShowing) processAlertModalQueue();
}

function processAlertModalQueue() {
...
async function loadDeploymentConfig() {
    try {
        ...
        document.getElementById('deployOsType').value = config.os_type || '';
        document.getElementById('deployProductionMode').checked = config.production_mode === true;
        document.getElementById('deployStrictMode').checked = config.strict_mode === true;
        document.getElementById('deployEnableAutoFix').checked = config.enable_auto_fix === true;
        document.getElementById('deployMonitorNetworkTraffic').checked = config.monitor_network_traffic !== false;
...
async function saveDeploymentConfig() {
    const data = {
        ...
        risk_check_interval: parseInt(document.getElementById('deployRiskInterval').value),
        admin_email: document.getElementById('deployAdminEmail').value,
        os_type: document.getElementById('deployOsType').value,
        production_mode: document.getElementById('deployProductionMode').checked,
        strict_mode: document.getElementById('deployStrictMode').checked,
        enable_auto_fix: document.getElementById('deployEnableAutoFix').checked,
        monitor_network_traffic: document.getElementById('deployMonitorNetworkTraffic').checked
    };
    try {
        const res = await fetchWithError('/api/deployment_config', {
            method: 'POST',
...
async function showLogs(id){
    ...
    const services = await res.json();
    const svc = services.find(s=>s.id===id);
    logTitle.innerText = `${svc?.name||id} 日志 (最后80行)`;
    modal.style.display = 'flex';
...
async function restartAll() {
    ...
    const now = Date.now();
    for (let svc of services) {
        pendingStopIds.add(svc.id);
        stopTimestamps.set(svc.id, now);
    }
    for (let svc of services) {
        try {
            const res = await fetchWithError(`/api/services/${svc.id}/restart`, { method: 'POST' });
            const data = await res.json();
            if (data.success) {
                showToast(`${svc.name} 重启成功`, 'success');
            } else {
                showToast(`${svc.name} 重启失败: ${data.message}`, 'error');
            }
        } catch (e) {
            showToast(`${svc.name} 重启请求失败: ${e.message}`, 'error');
        }
        pendingStopIds.delete(svc.id);
        stopTimestamps.delete(svc.id);
    }
    setTimeout(() => { refreshServices(); refreshMonitor(); }, 600);
}
...
async function loadRisks() {
    ...
    try {
        ...
        const data = await res.json();
        if (data.status === 'scanning') {
            container.innerHTML = '<div class="sensor-card" style="background:#fef9e3;">🔍 正在扫描系统风险，请稍候...<br><small>扫描可能需要几秒钟</small></div>';
            if (!riskPollingTimer) {
                riskPollingTimer = setInterval(() => loadRisks(), 2000);
            }
            return;
        }
        if (riskPollingTimer) {
            clearInterval(riskPollingTimer);
            riskPollingTimer = null;
        }
        const risks = data.risks || [];
        if (risks.length === 0) {
        ...
        if (riskPollingTimer) clearInterval(riskPollingTimer);
    }
}

async function manualScanRisks() {
    ...
    try {
        const res = await fetchWithError('/api/risks/scan', { method: 'POST' });
        const data = await res.json();
        if (data.status === 'started') {
            showToast('风险扫描已开始', 'info');
            loadRisks();
        } else {
            showToast(data.message || '扫描任务已在执行', 'warning');
        }
    } catch(e) {
...
async function loadChatHistory() {
    ...
    try {
        const res = await fetchWithError('/api/ai/history');
        const data = await res.json();
        ...
        for (const msg of history) {
            ...
            if (msg.role === 'user') {
                ...
                msgDiv.innerHTML = `<strong>你：</strong> ${escapeHtml(msg.content)}`;
                messagesDiv.appendChild(msgDiv);
            } else if (msg.role === 'assistant') {
                msgDiv.className = 'message ai';
                let renderedHtml;
                if (typeof marked !== 'undefined') {
                    renderedHtml = marked.parse(msg.content);
                } else {
                    renderedHtml = escapeHtml(msg.content).replace(/\n/g, '<br>');
                }
                msgDiv.innerHTML = `<strong>AI：</strong> ${renderedHtml}`;
                messagesDiv.appendChild(msgDiv);
        ...
        if (history.length === 0) {
            messagesDiv.innerHTML = '<div class="message system">你好！我是你的AI运维助手。我已经了解了当前服务器的状态（CPU、内存、磁盘、服务列表、风险等）。你可以问我任何关于服务器的问题，比如：“哪个服务占用内存最多？”、“如何优化磁盘空间？”等。</div>';
        }
...
async function sendAIMessage() {
    ...
    loadingDiv.innerHTML = '<strong>AI：</strong> 思考中...';
    ...
    try {
        ...
        let renderedHtml;
        if (typeof marked !== 'undefined') {
            renderedHtml = marked.parse(data.reply);
        } else {
            renderedHtml = escapeHtml(data.reply).replace(/\n/g, '<br>');
        }
        aiMsgDiv.innerHTML = `<strong>AI：</strong> ${renderedHtml}`;
        ...
        errorDiv.innerHTML = `<strong>错误：</strong> ${escapeHtml(error.message)}`;
...
async function init() {
    ...
    if (manualBtn) manualBtn.onclick = manualScanRisks;
